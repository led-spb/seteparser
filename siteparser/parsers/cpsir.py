...
class CpsirParser(SiteParser):
    ...
    def parse_slots(self, day_id):
        items = []
        params = {'j_idt13': 'j_idt13', 'j_idt13:j_idt17': '', 'javax.faces.ViewState': self.viewState, day_id: ''}
        req = self.sess.post('https://dms.i-t-systems.ru/cpsir/patient/selectDay.xhtml', data=params)

        filter_text = re.compile(
            u'<button id="(.*?)".*type="submit"><span class="ui-button-text">(.*?) время.*?<\/span>', re.I + re.M)
    ...
    def parse_doctors(self, speciality, spec_id):
        ...
        filter_text = re.compile(
            u'<button id="(.*?)".*?selector" type="submit"><span class="ui-button-text">(.*?) НОМЕР.*?<\/span',
            re.I + re.M)
        for m in filter_text.finditer(req.text):
            doctor_id = m.group(1)
            doctor = m.group(2)
    ...
    def parse_specs(self, speciality, viewAll=False):
        ...
        filter_text = re.compile(u'<button id="([^"]*)".*?<span class="ui-button-text">([^ ]+)[^<]*<\/span>',
                                 re.I + re.M)
        for m in filter_text.finditer(req.text):
            id = m.group(1)
            text = m.group(2)
